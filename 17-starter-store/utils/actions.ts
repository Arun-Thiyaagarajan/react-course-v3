...
const adminProductsURL: string = '/admin/products';
...
export const deleteProductAction = async (prevState: { productId: string }) => {
  ...
  try {
    ...
    revalidatePath(adminProductsURL);
...
export const fetchAdminProductDetails = async (productId: string) => {
  ...
  if (!product) redirect(adminProductsURL);
  return product;
};
...
export const toggleFavouriteAction = async (prevState: {
  ...
  try {
    ...
    return { message: favouriteId ? 'Removed from Favourites' : 'Added to Favourites' };
  } catch (error) {
    return renderError(error);
  }
};
